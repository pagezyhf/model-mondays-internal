# Now in Foundry: VibeVoice-ASR, MiniMax-M2.5, and Qwen3.5-9B

This week's Model Mondays edition features three models available in Microsoft Foundry: **Microsoft's VibeVoice-ASR**, a unified speech-to-text model that handles 60-minute audio files in a single pass with built-in speaker diarization and timestamps; **MiniMaxAI's MiniMax-M2.5**, a frontier agentic model that leads on coding and tool-use benchmarks with performance comparable to the strongest proprietary models at a fraction of their cost; and **Qwen's Qwen3.5-9B**, the largest of the Qwen3.5 Small Series, four models from 0.8B to 9B, that brings Qwen 3.5 to consumer-grade hardware.

All three represent a shift toward long-context, multi-step capability: VibeVoice-ASR processes up to an hour of continuous audio without chunking; MiniMax-M2.5 handles complex, multi-phase agentic tasks more efficiently than its predecessor—completing SWE-Bench Verified 37% faster than M2.1 with 20% fewer tool-use rounds; and Qwen3.5-9B brings multimodal reasoning that outperforms much larger models.

## Models of the week

---

### Microsoft: VibeVoice-ASR

**Model Specs**
- Parameters / size: ~8.3B (BF16)
- Primary task: Automatic Speech Recognition with diarization and timestamps (audio-text-to-text)

**Why it's interesting (Spotlight)**
- **60-minute single-pass with full speaker attribution**: VibeVoice-ASR processes up to 60 minutes of continuous audio without chunk-based segmentation—yielding structured JSON output with start/end timestamps, speaker IDs, and transcribed content for each segment. This eliminates the speaker-tracking drift and semantic discontinuities that chunk-based pipelines introduce at segment boundaries.
- **Joint ASR, diarization, and timestamps in one model**: Rather than running separate systems for transcription, speaker separation, and timing, VibeVoice-ASR produces all three outputs in a single forward pass. Users can also inject customized hotwords—proper nouns, technical terms, or domain-specific phrases—to improve recognition accuracy on specialized content without fine-tuning.
- **Multilingual with native code-switching**: Supports 50+ languages with no explicit language configuration required, and handles code-switching within and across utterances natively. This makes it suitable for multilingual meetings and international call center recordings without pre-routing audio by language.
- **Benchmarks**: On the [Open ASR Leaderboard](https://huggingface.co/spaces/hf-audio/open_asr_leaderboard), VibeVoice-ASR achieves an average WER of 7.77% across 8 English datasets (RTFx 51.80), including 2.20% on LibriSpeech Clean and 2.57% on TED-LIUM. On the [MLC-Challenge](https://huggingface.co/papers/2601.18184) multi-speaker benchmark: DER 4.28%, cpWER 11.48%, tcpWER 13.02%.

**Try it**

Microsoft built a [Hugging Face Spaces](https://huggingface.co/spaces/microsoft/VibeVoice-ASR) for you to try the model for yourself.

<div align="center" id="vibevoice-asr">
  <video src="https://github.com/user-attachments/assets/acde5602-dc17-4314-9e3b-c630bc84aefa" controls>
    Your browser does not support the video tag.
  </video>
</div>

---

### MiniMaxAI: MiniMax-M2.5

**Model Specs**
- Parameters / size: ~229B (FP8, Mixture of Experts)
- Primary task: Text generation (agentic coding, tool use, search)

**Why it's interesting (Spotlight)**
- **Leading coding benchmark performance**: Scores 80.2% on SWE-Bench Verified and 51.3% on Multi-SWE-Bench across 10+ programming languages (Go, C, C++, TypeScript, Rust, Python, Java, and others). In evaluations across different agent harnesses, M2.5 scores 79.7% on Droid and 76.1% on OpenCode—both ahead of Claude Opus 4.6 (78.9% and 75.9% respectively). The model was trained across 200,000+ real-world coding environments covering the full development lifecycle: system design, environment setup, feature iteration, code review, and testing.
- **Expert-level search and tool use**: M2.5 achieves industry-leading performance in BrowseComp, Wide Search, and Real-world Intelligent Search Evaluation (RISE), laying a solid foundation for autonomously handling complex tasks.
- **Professional office work**: Achieves a 59.0% average win rate against other mainstream models in financial modeling, Word, and PowerPoint tasks, evaluated via the GDPval-MM framework with pairwise comparison by senior domain professionals (finance, law, social sciences). M2.5 was co-developed with these professionals to incorporate domain-specific tacit knowledge—rather than general instruction-following—into the model's training.

![MiniMax-M2.5 Benchmark Performance](https://file.cdn.minimax.io/public/2003295c-001c-4381-af89-8859c197b5a0.png)

**Try it**

Software development scenario: You're building an automated code review and feature implementation system for a backend engineering team. Deploy MiniMax-M2.5 in Microsoft Foundry with access to your repository's file system tools and test runner. Given a GitHub issue describing a new API endpoint requirement, have the model first write a functional specification decomposing the requirement into sub-tasks, then implement the endpoint across the relevant service files, write unit tests with at least 85% coverage, and return a pull request summary explaining each code change and its relationship to the original requirement. Flag any implementation decisions that deviate from the patterns found in the existing codebase.

---

### Qwen: Qwen3.5-9B

**Model Specs**
- Model: Qwen/Qwen3.5-9B
- Parameters / size: 9B (BF16)
- Context length: 262,144 tokens natively; extensible to 1,010,000 tokens
- Primary task: Image-text-to-text (multimodal reasoning)

**Why it's interesting (Spotlight)**
- **Largest of the Qwen3.5 Small Series, with the full lineup available**: Qwen3.5-9B is the top of a four-model series—Qwen3.5-0.8B, Qwen3.5-2B, Qwen3.5-4B, and Qwen3.5-9B—all Apache 2.0 licensed, natively vision-capable, and sharing the same 262K-token context window. The series gives developers a consistent capability ladder from sub-1B on-device inference up to a 9B that fits in approximately 6GB of VRAM with 4-bit quantization, without switching model families.
- **Multimodal reasoning above larger competitors**: On MMMU-Pro, a challenging multimodal academic benchmark, Qwen3.5-9B scores 70.1%—above Gemini-2.5-Flash-Lite (59.7%) and Qwen3-VL-30B-A3B (63.0%). It also scores 78.9% on MathVision and 85.7% on MathVista (mini), reflecting tight spatial and diagrammatic reasoning from early-fusion multimodal training rather than a bolted-on vision adapter.

![Qwen3.5 Artificial Analysis](https://cdn.sanity.io/images/6vfeftx9/articles/09c6dab18a09248eee13cf3bf51df4e3d32b2f76-4893x3829.png)


**Try it**

Document intelligence scenario: You're building a financial reporting assistant for an analyst team. Deploy Qwen3.5-9B in Microsoft Foundry with vision enabled. Given a multi-page earnings report containing charts, tables, and narrative sections, have the model extract the three most significant year-over-year changes visible in the financial charts, cross-reference them against the narrative commentary to identify any discrepancies, and return a structured summary with a confidence rating for each finding. The 262K-token context window means the full document—including images—can be processed in a single call without splitting or chunking.

---

## Getting started

You can deploy open-source Hugging Face models directly in Microsoft Foundry by browsing the Hugging Face collection in the Foundry model catalog and deploying to managed endpoints in just a few clicks. You can also start from the Hugging Face Hub. First, select any supported model and then choose "Deploy on Microsoft Foundry", which brings you straight into Azure with secure, scalable inference already configured. Learn how to discover models and deploy them using Microsoft Foundry documentation.

* [Follow along the Model Mondays series and access the GitHub to stay up to date on the latest](https://github.com/microsoft/model-mondays/blob/main/docs/hugging-face/README.md)
* [Read Hugging Face on Azure docs](https://huggingface.co/docs/microsoft-azure/en/index)
* [Learn about one-click deployments from the Hugging Face Hub on Microsoft Foundry](https://huggingface.co/docs/microsoft-azure/en/guides/one-click-deployment-foundry)
* [Explore models in Microsoft Foundry](https://ai.azure.com/catalog/publishers/hugging%20face,huggingface)
